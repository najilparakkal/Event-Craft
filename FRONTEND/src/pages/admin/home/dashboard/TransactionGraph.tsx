...
const TransactionGraph: React.FC<GraphProps> = ({ usersDetails = [], vendorsDetails = [], revenue = [] }) => {
    ...
    const downloadInvoice = async () => {
        const invoiceData: any = {
            ...
            "products": [
                {
                    "quantity": userCounts[timePeriod].reduce((a, b) => a + b, 0),
                    "description": `User Registrations (${timePeriod})`
                },
                {
                    "quantity": vendorCounts[timePeriod].reduce((a, b) => a + b, 0),
                    "description": `Vendor Registrations (${timePeriod})`
                },
                {
                    "quantity": 1,
                    "description": `Total Revenue (${timePeriod})`,
                    "price": revenueCounts[timePeriod].reduce((a, b) => a + b, 0),
                }
            ],
            "bottom-notice": "This invoice represents the summary of your transaction graph data.",
        };
    ...
    return (
        <div className="w-full rounded-lg shadow bg-[#292F45] p-4 md:p-6 m-3">
            ...
            <div className="grid grid-cols-1 items-center border-gray-200 border-t dark:border-gray-700 justify-between mt-2.5">
                <div className="pt-5">
                    <a href="#" className="px-5 py-2.5 text-sm font-medium text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={downloadInvoice}>
                        <svg className="w-3.5 h-3.5 text-white me-2 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 20">
                            <path d="M14.066 0H7v5a2 2 0 0 1-2 2H0v11a1.97 1.97 0 0 0 1.934 2h12.132A1.97 1.97 0 0 0 16 18V2A1.97 1.97 0 0 0 14.066 0ZM7.5 14.17v-5.5l4.33 2.75-4.33 2.75Z" />
                            <path d="M0 5h5V0" />
                        </svg>
                        Download report
                    </a>
