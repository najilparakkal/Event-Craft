...
import { acceptRequest, cancelRequest, fetchRequest } from '../../../API/services/vendor/services';
...
const Requests: React.FC = () => {
  ...
  const confirmCancel = async () => {
    if (selectedRequestId !== null) {
      try {
        ...
        setReqs(reqs.filter(req => req.userId !== selectedRequestId));
      } catch (error) {
  ...
  const accept = async (userId: string) => {
    try {
      const response = await acceptRequest(userId, _id + "");
      if (response) {
        toast.success('ACCEPTED');
        setReqs(reqs.filter(req => req.userId !== userId));
      } else {
        ...
        {reqs.length === 0 ? (
          ...
          reqs.map((req, index) => (
            <div key={index} className="flex items-center bg-gray-400 rounded-md p-2 mb-2 space-x-4">
              ...
              <button className="text-red-600 ml-auto" onClick={() => accept(req.userId)}>Accept</button>
              <button className="text-red-600 ml-auto" onClick={() => reject(req.userId)}>Reject</button>
            </div>
          ))
        )}
